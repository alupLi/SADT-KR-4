...
- `test_create_user_success` - успешное создание пользователя
- `test_create_user_invalid_data` - неверные данные при создании
- `test_get_user_success` - получение существующего пользователя
- `test_get_user_not_found` - получение несуществующего (404)
- `test_get_all_users` - получение всех пользователей
- `test_update_user_success` - успешное обновление
- `test_update_user_not_found` - обновление несуществующего
- `test_update_user_partial` - частичное обновление
- `test_delete_user_success` - успешное удаление
- `test_delete_user_not_found` - удаление несуществующего
- `test_create_two_users_unique_ids` - уникальные ID
- `test_full_user_lifecycle` - полный цикл (интеграционный)
- `test_multiple_users_operations` - работа с несколькими пользователями
...
✅ Тесты асинхронные (pytest-asyncio + httpx.AsyncClient + ASGITransport)
✅ Faker используется для генерации данных
...
✅ Состояние изолировано (очистка после тестов)
✅ Все тесты проходят
